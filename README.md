...
Серверная часть на REST API на Django (модели, права доступа, пагинация, фильтрация). Фронтенд —   React с маршрутизацией и запросами к API. Письма и уведомления могут отправляться через очередь Celery + Redis или синхронно.
